    ...
    return {
        ...
        getParams:function(data){
            ...
            if (params.unix_timestamps=="TRUE" || params.unix_timestamps=="true"){  //toUpperCase fails when unix_timestamps is null
                out={
                    starttimestamp:Math.round(dateToUTC(params.starttime).getTime() / 1000) || Math.round(dateToUTC(data.query_starttime).getTime() / 1000),
                    endtimestamp:Math.round(dateToUTC(params.endtime).getTime() / 1000) || Math.round(dateToUTC(data.query_endtime).getTime() / 1000),
        ...
        readJson:function(wrap){
            ...
            environment.bgplay = new Bgplay({
                cur_instant:new Instant({
                    id:0,
                    timestamp: Math.round(dateToUTC(wrap.query_starttime).getTime() / 1000)
                }),
                starttimestamp: Math.round(dateToUTC(wrap.query_starttime).getTime() / 1000),
                endtimestamp: Math.round(dateToUTC(wrap.query_endtime).getTime() / 1000),
                type:"bgp"
            });
            ...
            function createEvents(uniquePAth,wrap){
                ...
                for (n=0; n<length; n++){
                    ...
                    instant = new Instant({id:event_id, timestamp: Math.round(dateToUTC(event.timestamp).getTime() / 1000), environment:environment});
