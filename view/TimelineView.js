...
var TimelineView=Backbone.View.extend({
    ...
    initialize:function(){
        ...
        this.eventAggregator.on("animationSpeedChanged",function(value){
    ...
    drawSelectionChart:function(nextFirstStep){
        ...
        while (nextEvent!=null && drawnEvents>0){
            ...
            if (sameTimestampEvent[0]!=null && sameTimestampEvent[0].get("instant").get("timestamp")!=nextEvent.get("instant").get("timestamp")){ //If the next event isn't in the same second
                ...
                drawWarp(this.fileRoot, position,timeWarpWidth,this,nextEvent.get("instant").get("timestamp")-sameTimestampEvent[0].get("instant").get("timestamp")); //Draw the temporal warp (arrows)
        ...
        if (nextEvent==null){//prevEvent is the last one event of the whole analyzed period
            ...
            if (finalInterval>0){
                drawWarp(this.fileRoot, position, timeWarpWidth, this, finalInterval);
            }
        }
        ...
        function drawWarp(fileRoot, position,width,env,offset){ //width can be useful with a more complex warp representation
            var image=fileRoot+"lib/img/bgplay/warp.gif";
            var posY;
    ...
    drawIntervalOnSelectionCanvas:function(){
        ...
        imageLeft=this.fileRoot+"lib/img/bgplay/leftSlider.png";
        imageRight=this.fileRoot+"lib/img/bgplay/rightSlider.png";
        var $this=this;
