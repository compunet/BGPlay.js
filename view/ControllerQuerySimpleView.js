...
var ControllerQuerySimpleView=Backbone.View.extend({
    ...
    render:function(){
        ...
        this.starttimestampPicker.datetimepicker("setDate",new Date(this.environment.params.starttimestamp*1000));

        this.endtimestampPicker.datetimepicker({
            changeMonth: true,
            changeYear: true,
            defaultDate:new Date(this.environment.params.starttimestamp*1000),
            hideIfNoPrevNext:true,
            timeFormat:'HH:mm:ss',
            separator:' ',
            dateFormat:'yy-mm-dd',
        ...
        this.endtimestampPicker.datetimepicker("setDate",new Date(this.environment.params.endtimestamp*1000));
    ...
    mouseOverFlag:function(){
        if (this.slideOpened==false)
            this.controlPanelDivFlagIco.attr('src',this.fileRoot+'lib/img/bgplay/openSlide.png');
    },
    mouseOutFlag:function(){
        if (this.slideOpened==false)
            this.controlPanelDivFlagIco.attr('src',this.fileRoot+'lib/img/bgplay/config.png');
    },
        ...
        if (this.slideOpened==false){
            ...
            this.dom.animate({height:'+=340'}, 600, function() {
                $this.dom.animate({width:'+=150'},300,function(){
                    ...
                    $this.controlPanelDivFlagIco.attr('src',$this.fileRoot+'lib/img/bgplay/closeSlide.png');
                    if ($this.controlPrefixDiv.length>0)
    ...
    updateConfig:function(){
        ...
        if (this.validateAll()==true){
            ...
            if (this.environment.thisWidget!=null){
                ...
                internalParams.starttimestamp = this.starttimestampPicker.datetimepicker("getDate").getTime()/1000;
                internalParams.endtimestamp = this.endtimestampPicker.datetimepicker("getDate").getTime()/1000;
